...
(function () {
  ...
  /* =========================================================
     OUTLOOK-SAFE PUBLISH KEY DIALOG

     window.prompt() is not supported inside Outlook add-in
     task panes. Use a small in-pane dialog instead.
  ========================================================= */
  ...
  function getPublishSecret() {
    ...
    if (stored) {
      return Promise.resolve(stored.trim());
    }
    ...
    return new Promise(function (resolve) {
      ...
      overlay.setAttribute("aria-labelledby", "pdPublishKeyTitle");
      ...
      description.style.cssText =
        "font-size:13px;line-height:1.5;color:#C9C3B8;margin-bottom:16px;";

      const input = document.createElement("input");
      input.id = "pdPublishKeyInput";
      input.type = "password";
      input.autocomplete = "off";
      input.spellcheck = false;
      input.setAttribute("aria-label", "Dispatch publishing key");
      input.style.cssText = [
        "display:block",
        "width:100%",
        "box-sizing:border-box",
      ...
      error.id = "pdPublishKeyError";
      error.style.cssText =
        "display:none;color:#F2EEE5;font-size:12px;margin-top:8px;";
  ...
  function looksLikeOptionalModule(element) {
    ...
    return /module|section|block|figure|image|text|reflection|studio|invite|question|pleasure|hero/.test(marker);
  }
  ...
  function cleanPublishHtml(html) {
    ...
    candidates.forEach(function (element) {
      ...
      if (!hasMeaningfulText(element) && !hasMeaningfulImage(element)) {
        element.remove();
      }
    });
  ...
  async function publishDispatch() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "Publish service returned HTTP " + response.status + "."
        );
      }
      ...
      if (!result || result.status !== "published") {
        if (result && result.message === "Unauthorized.") {
          ...
          throw new Error(
            "Publishing key rejected. Click Publish again and enter the current key."
          );
        }
