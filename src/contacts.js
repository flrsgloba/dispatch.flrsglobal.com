...
(function () {
  ...
  function setStatus(message) {
    ...
    else console.log("[Pleasure Dispatch Contacts]", message);
  }
  ...
  function normalizeContact(contact, index) {
    ...
    return {
      id: String(contact.id || key), name: name || email, email: email, status: status,
  ...
  function injectUi() {
    ...
    section.innerHTML =
      ...
      '<div class="pd-contact-toolbar"><select id="pdContactFile" aria-label="Select contact CSV"><option value="">Select a CSV from Google Drive…</option></select><button id="pdRefreshContacts" class="secondary" type="button">Refresh</button></div>' +
      '<div class="pd-contact-meta" id="pdContactMeta">Choose the CSV you want to use for this Dispatch.</div>' +
      '<div class="pd-contact-actions"><label class="pd-check-all"><input id="pdSelectAll" type="checkbox"> Select all active</label><input id="pdContactSearch" class="pd-contact-search" type="search" placeholder="Search contacts…" autocomplete="off"></div>' +
      '<div id="pdContactList" class="pd-contact-list"><div class="pd-contact-empty">No contact list selected.</div></div>' +
      '<div class="pd-contact-footer"><strong id="pdSelectedCount">0 selected</strong><span>Recipients will be added to Outlook BCC.</span></div>';
    ...
    document.getElementById("pdSelectAll").addEventListener("change", function () {
      filteredContacts().forEach(function (contact) {
        if (contact.active && contact.email) this.checked ? state.selected.add(contact.email) : state.selected.delete(contact.email);
      }, this);
      renderContacts();
    });
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .pd-contact-actions{display:flex;align-items:center;gap:12px;margin-bottom:10px}.pd-check-all{font:11px Arial,Helvetica,sans-serif;color:#C9C3B8;white-space:nowrap}.pd-contact-search{max-width:240px;margin-left:auto}
  ...
  async function loadContactFiles() {
    ...
    } catch (error) { setStatus("Could not load contact lists: " + error.message); }
  ...
  async function loadContacts(fileId) {
    ...
    state.loadedFileName = file ? file.fileName : "Contact CSV"; renderContacts(); setStatus("Loading contacts from " + state.loadedFileName + "…");
    try {
      const result = await apiGet({ action:"contacts", fileId:fileId });
      state.contacts = (Array.isArray(result.contacts) ? result.contacts : []).map(normalizeContact).filter(function (contact) { return !!contact.email; });
      ...
      renderContacts(); setStatus("Contacts loaded: " + active + " active.");
  ...
  function filteredContacts() {
    ...
    return state.contacts.filter(function (contact) { return (contact.name + " " + contact.email).toLowerCase().indexOf(search) !== -1; });
  }

  function renderContacts() {
    const list = document.getElementById("pdContactList"), count = document.getElementById("pdSelectedCount"), selectAll = document.getElementById("pdSelectAll");
    ...
    if (!contacts.length) list.innerHTML = '<div class="pd-contact-empty">No contacts match the current search.</div>';
      ...
      const checkbox = document.createElement("input"); checkbox.type = "checkbox"; checkbox.disabled = !contact.active; checkbox.checked = state.selected.has(contact.email);
      checkbox.addEventListener("change", function () { if (checkbox.checked) state.selected.add(contact.email); else state.selected.delete(contact.email); updateContactCounts(); });
  ...
  function setRecipientsAndBuild() {
    ...
    item.bcc.setAsync(emails, function (result) {
      ...
      if (typeof window.buildNewsletterHtml === "function" && typeof window.buildInOutlook === "function") window.buildInOutlook();
    });
  }
  ...
  function wrapBuild() {
    ...
    window.buildInOutlook = function () {
      if (selectedEmails().length) { setRecipientsAndBuild(); return; }
  ...
  function init() {
    injectUi(); loadContactFiles(); wrapBuild(); addUnsubscribeToNewsletterBuilder();
    setTimeout(wrapBuild,250); setTimeout(wrapBuild,1000); setTimeout(addUnsubscribeToNewsletterBuilder,250); setTimeout(addUnsubscribeToNewsletterBuilder,1000);
  }
